...
// Demonstrate registering a user-defined Arrow compute function outside of the
// Arrow source tree

namespace cp = ::arrow::compute;

template <typename TYPE, typename = typename std::enable_if<
                             arrow::is_number_type<TYPE>::value |
...
const cp::FunctionDoc func_doc{"User-defined-function usage to demonstrate "
                               "registering an out-of-tree function",
                               "returns x + y + z",
                               {"x", "y", "z"},
                               "UDFOptions"};
...
arrow::Status Execute() {
  ...
  ARROW_ASSIGN_OR_RAISE(auto x,
                        GetArrayDataSample<arrow::Int64Type>({1, 2, 3}));
  ARROW_ASSIGN_OR_RAISE(auto y,
                        GetArrayDataSample<arrow::Int64Type>({4, 5, 6}));
  ARROW_ASSIGN_OR_RAISE(auto z,
                        GetArrayDataSample<arrow::Int64Type>({7, 8, 9}));
